...
interface MinesSessionModalProps {
  isOpen: boolean;
  sessionData: MinesResult | null;
...
export function MinesSessionModal({
  isOpen,
  sessionData,
  isProcessing,
  error,
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl">
        ...
        <div className="space-y-3 mb-6 bg-slate-800/50 rounded-lg p-4">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="text-white font-semibold">{sessionData.minesCount}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-slate-400">Gems Found:</span>
            <span className="text-white font-semibold">{sessionData.clickedTiles?.length || 0}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-slate-400">Current Multiplier:</span>
            <span className="text-emerald-400 font-bold text-lg">{sessionData.currentMultiplier?.toFixed(2)}x</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-slate-400">Potential Payout:</span>
            <span className="text-emerald-400 font-bold text-lg">
              ₱{((sessionData.betAmount || 0) * (sessionData.currentMultiplier || 1)).toFixed(2)}
            </span>
          </div>
        </div>
